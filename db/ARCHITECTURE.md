...
```
plupdate-db           public  - vendor JSON + worker
plupdate              private - macOS app
...
1. **User submission** - the macOS app POSTs to a Cloudflare Worker
   that files an issue in `plupdate-submissions`. A maintainer
   promotes the good ones into a PR here.
